...
struct ImGuiDockNode
{
    ...
    {
        return (LocalFlags & ImGuiDockNodeFlags_DockSpace) != 0;
    }
    bool                    IsFloatingNode() const
    {
        return ParentNode == NULL && (LocalFlags & ImGuiDockNodeFlags_DockSpace) == 0;
    }
    bool                    IsCentralNode() const
    {
        return (LocalFlags & ImGuiDockNodeFlags_CentralNode) != 0;
    }
    ...
    {
        return (LocalFlags & ImGuiDockNodeFlags_NoTabBar) != 0;
    ...
    ImGuiDockNodeFlags      GetMergedFlags() const
    {
        return SharedFlags | LocalFlags;
    }
    ImRect                  Rect() const
    {
...
struct ImGuiViewportP : public ImGuiViewport
{
    ...
    ImGuiViewportP()
    {
        Idx = -1; LastFrameActive = LastFrameDrawLists[0] = LastFrameDrawLists[1] = LastFrontMostStampCount = -1; LastNameHash = 0; Alpha = LastAlpha = 1.0f; PlatformMonitor = -1; PlatformWindowCreated = false; Window = NULL; DrawLists[0] = DrawLists[1] = NULL; LastPlatformPos = LastPlatformSize = LastRendererSize = ImVec2(FLT_MAX, FLT_MAX);
    }
    ~ImGuiViewportP()
    {
    ...
    {
        return ImRect(Pos.x + WorkOffsetMin.x, Pos.y + WorkOffsetMin.y, Pos.x + Size.x + WorkOffsetMax.x, Pos.y + Size.y + WorkOffsetMax.y);
    }
...
struct ImGuiWindowSettings
{
    ...
    ImGuiWindowSettings()
    {
        ID = 0; Pos = Size = ViewportPos = ImVec2ih(0, 0); ViewportId = DockId = ClassId = 0; DockOrder = -1; Collapsed = WantApply = false;
    }
    char* GetName()
    {
        return (char*)(this + 1);
...
public:
    ...
    {
        ImGuiContext& g = *GImGui; float scale = g.FontBaseSize * FontWindowScale * FontDpiScale; if (ParentWindow) scale *= ParentWindow->FontWindowScale; return scale;
    }
...
struct ImGuiTabItem
{
    ...
    ImGuiTabItem()
    {
        ID = 0; Flags = ImGuiTabItemFlags_None; Window = NULL; LastFrameVisible = LastFrameSelected = -1; NameOffset = -1; Offset = Width = ContentWidth = 0.0f; WantClose = false;
    }
};
...
namespace ImGui
{
    ...
    {
        while (node->ParentNode) node = node->ParentNode; return node;
    }
    inline int              DockNodeGetDepth(const ImGuiDockNode* node)
    {
        int depth = 0; while (node->ParentNode)
        {
    ...
    {
        ImGuiContext& g = *GImGui; return g.CurrentWindow->DockNode;
    }
    ...
    {
        ImGuiDockNode* node = DockBuilderGetNode(node_id); if (!node) return NULL; return DockNodeGetRootNode(node)->CentralNode;
    }
